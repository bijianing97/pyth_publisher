      ...
      (onOpen = () => {
        ...
        interval = setInterval(() => {
          if (!timeOut) {
            ws.ping();
            timeOut = setTimeout(() => {
              logger.debug("WebSocket", "loss connection...");
...
export class JSONRPCWebSocket extends EventEmitter {
  ...
  private onMessage = (data: WS.RawData) => {
    try {
      ...
      // check format
      if (jsonrpc !== "2.0") {
        logger.warn("JSONRPCWebSocket", "invalid JSON RPC version:", jsonrpc);
        return;
      }
      ...
      if (id !== undefined) {
        ...
        if (request) {
          ...
          logger.warn("JSONRPCWebSocket", "unknown response");
        }
        return;
      }
      ...
      logger.warn("JSONRPCWebSocket", "unknown data");
    } catch (err) {
      logger.error("JSONRPCWebSocket", "error:", err);
    }
  };
